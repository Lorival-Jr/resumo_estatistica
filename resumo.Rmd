...
$f(x) \ge 0$ e $\int^{+\infty}_{-\infty}f(x) = 1$
...
Representa a distribuição amostral da média.

### Estimadores

<p>![](img/estimadores_precisao.png)</p>
**Propriedades**<br>

- Não tendencioso (ou não viciado)<br>
&emsp; $E(\hat\theta) = \theta$
- Tendencioso (Viciado)<br>
&emsp; Vicio: $B(\hat\theta)=E(\hat\theta)-\theta$<br>
- Eficiência 
&emsp;$Var(\hat\theta_1)<Var(\hat\theta_2)$ (menor melhor)

OBS: Se $\hat\theta_1$ é não viciado e tiver variância mínima, dizemos que $\hat\theta$ é ENVVM (Estimador não viciado de Var mínima)
...
5. Verificar se $\hat\theta$ encontrado em (4) é ponto de máximo, isso derivando novamente e vendo se é menor que zero.
...
- Distribuição amostral de $\theta$ (ou quantidade pivotal)
...
- $IC(\mu, 1 - \alpha)$ = $\bar{x} \pm Z_{\frac{\alpha}{2}}\cdot \frac{S}{\sqrt n}$

**b)Amostra pequena**

- $IC(\mu, 1 - \alpha)$ = $\bar{x} \pm t_{\left (\frac{\alpha}{2},n-1 \right )}\cdot \frac{S}{\sqrt n}$

Em que t é a distribuição t student com n-1 graus de liberdade

$$f(x) = \frac {\Gamma \left ( \frac{k + 1}{2} \right )}{\sqrt{\pi k} \Gamma{ \left ( \frac{k} {2} \right )}} \frac{1}{\left ( \frac{x^2}{k} + 1 \right ) \frac{k +1}2}$$
...
Para esse intervalo de confiança há duas formas de constuílo, a otimista e a conservadora:

$IC(p, 1-\alpha) = \hat p \pm Z_{\frac{\alpha}{2}} \cdot \sqrt{\frac{\hat p (1 - \hat p)} n}$ (otimista)

$IC(p, 1-\alpha)=\hat p \pm Z_{\frac{\alpha}{2}} \cdot \sqrt{\frac 1 {4n}}$ (conservador)

**Margem de Erro**<br>

- $e = Z_{\frac{\alpha}{2}} \sqrt{\frac{\hat{p}(1- \hat p)}{n}}$ (Otimista)
- $e = Z_{\frac{\alpha}{2}}\sqrt{\frac{1}{4n}}$
...
- $n = \left (  t_{\left ( \frac{\alpha}{2},n-1 \right )} \cdot \frac S e \right )^2$
...
2. Estabelecer a estatística teste e sua distribuição
3. Estabelecer o nível de significância e regiçoes críticas
...
Sempre $H_0$ sempre carrega o igual sendo $=,\ge,\le$.

#### Teste de hipótese proporção

$Zc = \frac{\hat p - p}{\sqrt{\frac{p(1-p)}{n}}}$ ~ $N(0,1)$

#### Cálculo do valor p:

Como o p-valor funciona?

Basicamente se o p-valor for menor que a significância rejeita-se $H_0$ 

(Pode ser aplicado a $t_c$, $\chi^2_c$, $F_c$, etc)
- Se $H_1: \theta \neq \theta_0$ => $p = 2(1- \Phi(z_c))$ 
- Se $H_1: \theta > \theta_0$ => $p = 1- \Phi(z_c)$ 
- Se $H_1: \theta < \theta_0$ => $p = \Phi(z_c)$ 
...
$$n = \left ( \frac{Z_{\alpha} \sqrt{\hat p(1- \hat p)} + Z_\beta \sqrt{p(1-p)}}{p - \hat p} \right )^2$$
...
#### Teste de hipóstese para $\sigma^2$

Se $Z_i$ ~ $N(0,1)$

$\sum^n_{i=1}(z_i-\bar{z})^2$ ~ $\chi^2_{n-1}$

e assim

$\chi^2_c = \frac{(n-1)S^2}{\sigma^2}$ ~ $\chi^2_{n-1}$
...
- Por padrão o raio da diferença é 4, mas pode ser ajustado dependendo da situação

Se $F_c \ge4$, então $\sigma^2_1 \neq \sigma^2_2$

Se $F_c <4$, então $\sigma^2_1 =\sigma^2_2$

#### Teste de hipóteses para $\mu_1 - \mu_2 (Diferença de médias)$

1. Se $\sigma_1$ e $\sigma_2$ são conhecidos, então

$z_c = \frac{\bar{x_1}-\bar{x_2}}{\sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}}}$ ~ $N(0,1)$

2. Se $\sigma_1$ e $\sigma_2$ são desconhecidos e iguais, então

$t_c = \frac{\bar{x_1}-\bar{x_2}}{s_p\sqrt{\frac{1}{n_1} + \frac{1}{n_2}}}$ ~ $t_{(n1 + n2 -2)}$
...
$t_c = \frac{\bar{x_1}-\bar{x_2}}{\sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}}$ ~ $t_{\phi}$

em que

$\phi = \frac{(\omega_1 + \omega_2)^2}{\frac{\omega_1^2}{n1-1} + \frac{\omega_2^2}{n2-1}}$;&emsp;
$\omega_1 = \frac{s^2_1}{n_1}$ &emsp;e&emsp; $\omega_2 = \frac{s^2_2}{n_2}$
...
&emsp; $s_d$ é o desvio padrão das diferenças
...
```
<p>Assim, com 0.95 de confiança, a estatística $t_c$ é igual 2.422, estando na região de rejeição, portanto rejeita-se $H_0$, apresentando evideências qdas diferênças de pressão antes e depois da aplicação do fármaco.</p>
...
$e$ = $\frac{marginal(x)\cdot marginal(y)}{total(n)}$

Como tomar a decisão?

$\chi^2_c \ge \chi_t^2$ $\rightarrow$ Rejeitar $H_0$

$\chi^2_c < \chi_t^2$ $\rightarrow$ Não rejeitar $H_0$
...
$\chi^2_t = \chi^2_{(r -1)(s-1); 0.05} = \chi^2_{(2 -1)(2-1); 0.05} = \chi^2_{1; 0.05} = 3.841$

Como $\chi^2_c>\chi_t^2$, então rejeitamos $H_0$, ou seja, com 95% de confiança, existem indícios de que dores e o programa não são independentes.
...
$t_c = r \sqrt{\frac{n-2}{1-r^2}}$ ~ $t_{n-2}$

Conclusão:

$|t_c| \le |t_t| \rightarrow$ não rejeita $H_0$

$|t_c| > |t_t| \rightarrow$ rejeita $H_0$
...
$S_{aE} = \sum_i(y_i - \hat{y_i})^2$

##### Inferência sobre os parâmetros do modelo

a) Para o coeficiente angular ($\hat{b}$)

$E_p(\hat{b}) = \sqrt{\frac{\sigma^2}{S_{xx}}}$

$\hat{b} \pm \sqrt{\frac{\sigma^2}{S_{xx}}}t_{(n-2); \frac \alpha 2}$

$t_c = \frac{\hat{b} - b}{\sqrt{\frac{\hat{\sigma^2}}{S_{xx}}}}$ ~ $t_{n-2}$

$H_0: \hat b = 0$

$H_1: \hat b \neq 0$

Se |$t_c$| > $t_{\frac{\alpha}{2}; n-2} \rightarrow$ rejeita $H_0$

b) Para o intercepto ($a$)

$EP(\hat{a}) = \sqrt{\hat \sigma^2 \left (\frac{1}{n} + \frac{\bar{x}^2}{S_{xx}} \right)}$

$\hat{a} \pm \sqrt{\hat \sigma^2 \left (\frac{1}{n} + \frac{\bar{x}^2}{S_{xx}} \right )} t_{\frac \alpha 2 ; n-2}$

$H_0: a = a_0$

$H_1: a \neq a_0$

$t_c = \frac{\hat a - a}{\sqrt{\hat \sigma^2 \left (\frac{1}{n} + \frac{\bar{x}^2}{ S_{xx}}\right )}}$

Se $|t_c| > t_{\frac \alpha 2 ; n-2}$ Rejeita-se $H_0$
...
Toda função que satisfaça as duas primeiras propridades é uma fdp.
...
$$Z = \frac{x - \mu}{\sigma}$$
e 
$$f(z)=\int^z_{-\infty}\frac{1}{\sqrt{2\pi}}e^{-\frac{z^2}2}$$
...
- $E(xy) = E(x)E(y)$, se x e y são independentes
...
### Probabilidade do valor Máximo e Mínimo
